...
<svelte:head>
	<title>{t(translations, 'nav.register')} - AutoFeedback</title>
...
<div class="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
	<div class="sm:mx-auto sm:w-full sm:max-w-md">
		...
		<p class="mt-2 text-center text-sm text-gray-600">
			...
			<a href="/{lang}/auth/login" class="font-medium text-primary hover:text-primary-dark">
				{t(translations, 'auth.signInHere')}
			</a>
		</p>
	</div>
		...
		<div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
			...
			<form method="POST" use:enhance={() => {
				...
				<div>
					...
					>
						{loading ? t(translations, 'common.loading') : t(translations, 'nav.register')}
					</button>
				</div>
			</form>
		</div>
